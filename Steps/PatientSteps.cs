...
namespace OpenEmrApplication.Steps
{
    ...
    public class PatientSteps
    {
     ...
     //   [Given(@"I have browser with google map url")]
        ...
        [When(@"I fill the detail from excel '(.*)' with sheetname '(.*)'")]
        public void WhenIFillTheDetailFromExcelWithSheetname(string path, string sheetname)
        {
            DataTable dt= ExcelUtils.SheetToDataTable(path, sheetname);
           ...
           // Console.WriteLine(dt.Rows[0]["username"]);
            ...
            for(int i=0;i<dt.Rows.Count;i++)
            {
                ...
                Console.WriteLine(dt.Rows[i]["language"]);
                Console.WriteLine(dt.Rows[i]["expectedvalue"]);
            }
            Console.WriteLine("---------------------------------------");
            foreach(DataRow row in dt.Rows)
            {
                ...
                Console.WriteLine(row["language"]);
                Console.WriteLine(row["expectedvalue"]);
            }

        }

        [Then(@"I should validate the submitted form")]
        public void ThenIShouldValidateTheSubmittedForm()
        {
